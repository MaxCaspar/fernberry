...
### Task (type="auto")
  ...
  ## ActionRun `vercel --yes` to deploy. Capture deployment URL.
...
### Task (type="auto")
  ...
  ## ActionUse Stripe API to create webhook endpoint at /api/webhooks. Save signing secret to .env.
  ## Verifystripe webhooks list shows endpoint, .env contains STRIPE_WEBHOOK_SECRET


### Task (type="checkpoint:human-verify")
  ## What BuiltStripe webhook configured
  ## How To VerifyCheck Stripe dashboard > Developers > Webhooks shows endpoint with correct URL
  ## Resume SignalType "yes" if correct
...
### Task (type="auto")
  ...
  ## ActionRun `upstash redis create myapp-cache --region us-east-1`. Save URL to .env.
  ## Verify.env contains UPSTASH_REDIS_URL, upstash redis list shows database
...
### Task (type="auto")
  ## NameBuild macOS app
  ## ActionRun `xcodebuild -project App.xcodeproj -scheme App build`. Check output for errors.
  ## VerifyBuild succeeds with "BUILD SUCCEEDED" in output


### Task (type="checkpoint:human-verify")
  ## What BuiltBuilt macOS app at DerivedData/Build/Products/Debug/App.app
  ## How To VerifyOpen App.app and check: login flow works, no visual glitches
...
```xml
### Task (type="auto")
  ## NameConfigure environment variables
  ## ActionWrite .env file with: DATABASE_URL, STRIPE_KEY, JWT_SECRET (generated).
  ## VerifyRead .env confirms all variables present
...
### Task (type="auto")
  ...
  ## Verifyvercel ls shows deployment
...
```xml
### Task (type="auto")
  ## NameCreate Stripe webhook endpoint
  ## ActionUse Stripe API to create webhook at /api/webhooks


<!-- If API returns 401 Unauthorized -->

### Task (type="checkpoint:human-action" gate="blocking")
  ## ActionProvide Stripe API key so I can continue webhook configuration
  ## Instructions
  ...
  ## VerificationStripe API key works: curl test succeeds
  ## Resume SignalType "done" or paste the key


<!-- After key provided, Codex writes to .env and continues -->

### Task (type="auto")
  ## NameSave Stripe key and create webhook
  ## Action
    1. Write STRIPE_SECRET_KEY to .env
    2. Create webhook endpoint via Stripe API
    3. Save webhook secret to .env
  
  ## Verify.env contains both keys, webhook endpoint exists
...
```xml
### Task (type="auto")
  ## NameCreate GitHub repository
  ## ActionRun `gh repo create myapp --public`


<!-- If gh returns "Not logged in" -->

### Task (type="checkpoint:human-action" gate="blocking")
  ## ActionAuthenticate GitHub CLI so I can create repository
  ## Instructions
    I need GitHub authentication to create the repo.
    Run: gh auth login
    Follow the prompts to authenticate (browser or token).
  
  ## Verificationgh auth status shows "Logged in"
  ## Resume SignalType "done" when authenticated


### Task (type="auto")
  ## NameCreate repository (authenticated)
  ## ActionRun `gh repo create myapp --public`
  ## Verifygh repo view shows repository exists
...
### Task (type="auto")
  ## NameCreate Upstash Redis database
  ## ActionRun `upstash redis create myapp-cache --region us-east-1`


<!-- If upstash returns auth error -->

### Task (type="checkpoint:human-action" gate="blocking")
  ## ActionConfigure Upstash CLI credentials so I can create database
  ## Instructions
  ...
  ## Verificationupstash auth status shows authenticated
  ## Resume SignalType "done" when authenticated


### Task (type="auto")
  ## NameCreate Redis database (authenticated)
  ## Action
    1. Run `upstash redis create myapp-cache --region us-east-1`
    2. Capture connection URL
    3. Write to .env: UPSTASH_REDIS_URL={url}
  
  ## Verifyupstash redis list shows database, .env contains URL
...
### Task (type="checkpoint:human-action" gate="blocking")
  ...
  ## Resume SignalType "done" when verified
