...
Complete guide for automatically installing MCP servers in both Codex and Codex Desktop with safe credential management.

## Overview

When you build an MCP server, you want it instantly available in both:
- **Codex** - For development and coding workflows
- **Codex Desktop** - For conversational usage
...
1. Adding to Codex via CLI (`codex mcp add`)
2. Editing Codex Desktop config JSON manually
...
3. Install in Codex (user scope)
4. Manually update Codex Desktop config with variable references
...
### Step 2: Install in Codex

```bash
# Source secrets
source ~/.mcp_secrets

# Install with actual values (Codex stores them securely)
codex mcp add --transport stdio meta-ads \
  --scope user \
...
**Note:** When using `codex mcp add`, you pass actual values. Codex stores them securely in `~/.Codex/.Codex.json` and references them correctly.

### Step 3: Configure Codex Desktop

Edit `~/Library/Application Support/Codex/codex_desktop_config.json`:
...
Restart Codex Desktop and test.
...
```bash
source ~/.mcp_secrets
codex mcp add --transport stdio stripe \
  --scope user \
...
```bash
source ~/.mcp_secrets
codex mcp add --transport stdio ghl \
  --scope user \
...
### Check Codex Desktop Configuration

```bash
# View all servers
cat ~/Library/Application\ Support/Codex/codex_desktop_config.json | jq '.mcpServers'

# Check specific server
cat ~/Library/Application\ Support/Codex/codex_desktop_config.json | jq '.mcpServers["meta-ads"]'

# Verify cwd property is set
cat ~/Library/Application\ Support/Codex/codex_desktop_config.json | jq '.mcpServers["meta-ads"].cwd'

# Verify env uses variable expansion
cat ~/Library/Application\ Support/Codex/codex_desktop_config.json | jq '.mcpServers["meta-ads"].env'
```
...
2. **Install in Codex** using `codex mcp add` with actual values
3. **Configure Codex Desktop** with variable expansion (`${VAR}`)
...
- Install Codex CLI: Open Codex → run `/install-cli`
...
~/.Codex/scripts/install-mcp.sh ...
```

**"Server not appearing in Codex Desktop"**
- Verify JSON syntax: `jq '.' ~/Library/Application\ Support/Codex/codex_desktop_config.json`
- Check backup file if config is corrupted
- Restart Codex Desktop

**"Environment variable not found"**
- Check `~/.Codex/.env` exists
...
**Codex:**
```bash
codex mcp add --transport stdio my-ts-server \
  --scope user \
  --env API_KEY=${API_KEY} \
...
```bash
# HTTP server
codex mcp add --transport http my-server https://api.example.com/mcp

# SSE server with headers
codex mcp add --transport sse my-server \
...
4. **Variable expansion everywhere** - Codex Desktop configs must use `${VAR}`
