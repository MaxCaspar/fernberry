...
**End state**: Server running in both Codex and Codex Desktop with all credentials configured.
</overview>
...
- [ ] Step 5: Install in Codex
- [ ] Step 6: Install in Codex Desktop
...
Ask the user in chat to collect all information upfront:
...
**SECURITY CRITICAL:** NEVER ask Lex to paste secrets into chat. Secrets must never go through OpenAI/Codex servers or appear in conversation history.
...
UV_PATH=$(which uv)
NODE_PATH=$(which node)
DESKTOP_CONFIG="$HOME/Library/Application Support/Codex/codex_desktop_config.json"
...
echo "✓ Installed in Codex Desktop"
echo "⚠️  IMPORTANT: Restart Codex Desktop for changes to take effect"
```
...
tail -50 ~/Library/Logs/Codex/mcp-server-{server-name}.log
```
...
**Final checklist:**
```
- [ ] Server appears in `codex mcp list` with ✓ Connected
...
codex mcp list | grep -q "{server-name}" && echo "✓ Installed in Codex" || echo "✗ Not installed"
```

**Step 6 validation:**
```bash
# Check Codex Desktop config
jq '.mcpServers | has("{server-name}")' "$HOME/Library/Application Support/Codex/codex_desktop_config.json"
# Should output: true
```
...
- Codex Desktop requires absolute paths
...
- Codex Desktop config is backed up automatically
- Can restore with: `cp codex_desktop_config.json.backup.<timestamp> codex_desktop_config.json`
