...
<critical_pattern>
**Why this matters:** API responses exhaust Codex's context window after just 5-10 operations. Response optimization achieves 85% token reduction and enables 100+ operations per conversation.
...
APIs return verbose responses with nested objects and metadata that Codex doesn't need.
...
```python
# What Codex ACTUALLY needs vs what API returns
FIELD_CONFIGS = {
...
**Key principle:** Include only what Codex needs to:
...
Document available fields in operation schema description or point to API docs. Codex can learn which fields exist through:
...
Codex: [receives 10,000 token response]
User: "Search for Beatles"
Codex: [receives 10,000 tokens]
... after 5-10 searches, context exhausted
Codex: "I've run out of context"
```

**Optimized:**
```
User: "Search for Queen"
Codex: [receives 1,500 token response]
User: "Search for Beatles"
Codex: [receives 1,500 tokens]
User: "List all my playlists"
Codex: [receives first 15k token chunk]
Codex: "Page 1/3 - call continue for more"
User: "continue"
Codex: [receives second chunk from cache]
