...
2. **Clear description**: Codex knows when to use it
3. **Strict schema**: Input validation prevents errors
4. **Predictable output**: Consistent response format
5. **Error messages**: Help Codex understand what went wrong
...
3. **Appropriate mime types**: Helps Codex understand content
...
The patterns above work well for small to medium servers (< 20 tools). However, if you're wrapping a large API (GitHub, Stripe, Slack, etc.), each tool definition consumes tokens in Codex's context window.
